...
# Estrutura do html
- Os arquivos da interface web estão localizados dentro da pasta templates/
...
- O arquivo [graphics_functions.py](https://github.com/LeticiaRosa05/Enem-mais/blob/main/graphics_functions.py) é responsável pela função da geração dos gráficos, bem como toda ação necessária para lidar com alterações nos filtros (parametrização), especifiações de quantidade de barras/linhas a serem exibidas e a lógica/escolha do tipo de gráfico a ser gerado (barras/linhas)
- Embora na [graphics_interface.html](https://github.com/LeticiaRosa05/Enem-mais/blob/main/templates/graphics_interface.html) seja possível escolher o gráfico desejado, nas páginas de gráficos estáticos ao se aplicar o filtro por cidade é gerado somente o gráfico de barras, uma vez que o gráfico de linhas se torna sem sentido se não abordar mais de um resultado (pois exibe somente um ponto, não há variação na altura da linha para que seja exibida)
